...
/*STARTING FROM LINE 30 UNTIL LINE 130

This code is a program that uses two lists (read_list and wait_list) to manage reading and writing to a pipe.
It contains functions for initializing the pipe (pipe_init), writing to the pipe (write_pipe),
and reading from the pipe (read_pipe). 
The read_pipe function will either return the requested value if it is already in 
the read_list, or it will create a request in the wait_list and wait for a writer to 
add the requested value to the read_list. The write_pipe function will add a value to 
the read_list and wake up any waiting readers in the wait_list that are requesting that value. 
The process_init function initializes the pipe and sets up the root process.
Reference:
Wang-GY. (2018). https://github.com/Wang-GY/pintos-project2 UWE Harvard style reference. December 22. */
/*Implemented by AA*/
...
/* Add this thread to the children list and make sure that the thread starts correctly. */
enum intr_level old_level = intr_disable();
...
int thread_id = thread_current()->tid;

// If load was successful, execute the command
if (success) {
  write_pipe(thread_id, EXEC, thread_id);

  // Put arguments into stack
  int i = argc;
  char* addr_arr[argc];
  printf("Trying to put args\n");
  ...
  // If load failed, set the exit status and quit
  write_pipe(thread_current()->tid, EXEC, TID_ERROR);
  thread_exit();
}

// Free command_bak and file_name
free(command_bak);
palloc_free_page(file_name);

// Print the contents of the stack for debugging purposes
...
char* extract_command(char* command, char* argv[], int* argc) {
  ...
  *argc = 0;
  ...
  // Copy command into command_bak
  strlcpy(command_bak, command, PGSIZE);

  // Tokenize command_bak and store the tokens in argv
